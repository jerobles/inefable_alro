...
0.9. **Pendientes del usuario (no van en la página, son tareas suyas).** Lista consolidada al cierre de la sesión del 2026-09-06 — el sitio quedó **funcionando y vendiendo**, con la primera venta real hecha y verificada en los cinco puntos (correo al cliente, aviso interno, hoja de Google, Brevo y Mercado Pago).
   ...
   - **Decidir si se hace el webhook de Mercado Pago** (diseñado en `docs/webhook-mercado-pago.md`, no implementado). Mientras no exista, la regla operativa es **confirmar cada pago en el panel de Mercado Pago por número de pedido antes de despachar** — la hoja dice "Pago: En línea", que solo significa que fue por ese camino, no que se aprobó.
