...
0. **En curso (2026-08-21): rediseño de talleres**, por módulos, aprobando cada uno con el usuario antes de seguir:
   ...
   **Pendiente de los 5 módulos de talleres, solo para cuando se publique:**
   - Agregar el "Outgoing webhook" de `newsletter-sync` en Forms (Netlify solo detecta el formulario "newsletter" después de desplegarlo), limitado específicamente a ese formulario.
   - Corregir el webhook existente de `brevo-sync`: hoy dispara con "any form", debe limitarse a "pre-inscripcion-curso" (con 3 formularios ahora — taller, newsletter, producto — "any form" mezclaría todo).
   - Igual, agregar el webhook de `producto-sync` (ver abajo), limitado a "pedido-producto".
   - `formresponses@netlify.com` sigue siendo el remitente fijo de las notificaciones nativas de Netlify Forms (en paralelo, no se puede personalizar).
...
0.5. **Catálogo "lite" (Fase 8 parcial, sin pago — 2026-08-22).** El usuario decidió el orden: catálogo primero, luego dominio (Fase 6), y dejar la tienda con pago real (Fase 8 completa, Shopify) para más adelante. Iniciativa completa en código, por los mismos módulos que talleres:
   ...
   - **Tienda-4** ✅ código hecho, `BREVO_PRODUCTOS_LIST_ID` = `7` ya configurada en Netlify — formulario "Quiero este producto" en `/tienda` → `netlify/functions/producto-sync.js` (mismo patrón que brevo-sync, atributo Brevo `PRODUCTO_INTERES`). Falta solo el paso de webhook de Forms (ver arriba) una vez publicado.
