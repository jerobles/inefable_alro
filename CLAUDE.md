...
0.9. **Pendientes del usuario (no van en la página, son tareas suyas):**
   - **Revisión legal** de `/privacidad` y `/terminos` con un abogado, y **registro de la base de datos en el RNBD de la SIC**. Las advertencias que decían esto en las páginas se retiraron el 2026-09-06 a pedido del usuario; el pendiente sigue vivo aquí.
   - **Crear talleres con fecha futura** desde `/admin`: no hay ninguno vigente (los dos cargados son de agosto y `/curso` filtra los vencidos), así que esa página muestra el estado vacío y el pago de talleres **nunca se ha podido probar**.
