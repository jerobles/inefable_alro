...
0. **En curso (2026-08-21): rediseño de talleres**, por módulos, aprobando cada uno con el usuario antes de seguir:
   ...
   - Módulo 1.5 ✅ **activo y confirmado en producción** — `brevo-sync.js` manda 2 correos por Brevo (transactional email, misma API key): confirmación al lead + notificación a la empresa con botón "Escríbele por WhatsApp" directo al número del lead. Remitente separado en dos para no bloquear todo por el correo corporativo:
     ...
     - `BREVO_SENDER_EMAIL` = `inefable.alro@gmail.com` ✅ configurada (2026-08-22) — remitente del correo de confirmación AL CLIENTE (antes pendiente, ya no). Falta confirmar que quedó verificada como sender en Brevo (paso del usuario).
...
- **Funciones de Netlify: nunca declarar `__dirname` propio.** El bundler (esbuild) ya inyecta uno al empaquetar la función, así que `const __dirname = path.dirname(fileURLToPath(import.meta.url))` compila bien en local pero tumba la función **en producción** con `SyntaxError: Identifier '__dirname' has already been declared` (HTTP 502). Para leer archivos junto a la función, usar `fs.readFileSync(fileURLToPath(new URL('./data/x.json', import.meta.url)))`. Mordió el 2026-08-22 en `pago-taller.js` y `pago-producto.js` — y ojo: **un dry-run local con `node` NO lo detecta**, porque el error solo aparece después del empaquetado. Hay que probar la función desplegada de verdad.
