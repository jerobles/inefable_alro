...
0.2. **Carrito de compras (2026-09-05).** Antes solo se podía pedir un producto por vez: tres velas distintas eran tres formularios, tres pagos y **tres domicilios**. Por módulos, como siempre:
   ...
   - **Carrito-3 (pendiente)** — hoja de Google con una fila por pedido (el usuario la quiere **además** de Netlify: Netlify para que quede guardado, la hoja para compartirla con su cliente), y retirar el formulario viejo de producto único de las páginas de producto (`FormPedido.astro`), que hoy sigue ahí como alternativa secundaria.
