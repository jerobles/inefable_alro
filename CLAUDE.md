...
- **Identidad y historial en Brevo (importante para entender el CRM).** El **correo es la identidad** del contacto: `POST /v3/contacts` con `updateEnabled: true` busca por correo. Recompra con el mismo correo → actualiza sin problema. El teléfono NO identifica, solo tiene regla de unicidad (ver punto siguiente).
  - Ojo con "actualizar": **sobrescribe**. Por eso `brevo-sync` y `producto-sync` primero leen el contacto (`getBrevoContact`) y **acumulan** con `acumularValor()`, que arma una lista `"A | B | C"` sin repetidos, con el más reciente al final, y con tope (12 ítems / 240 caracteres) porque los atributos de texto de Brevo tienen límite de tamaño. Sin esto, cada pedido borraba el anterior y no había forma de saber qué había comprado un cliente.
  - Atributos que mantiene el catálogo: `PRODUCTO_INTERES` (lista acumulada, con la presentación entre paréntesis), `PEDIDOS_TOTAL` (número, se incrementa siempre — no se pierde aunque la lista se recorte) y `ULTIMO_PEDIDO` (fecha `YYYY-MM-DD`). El de talleres acumula `TALLER` y también escribe `ULTIMO_PEDIDO`.
  - **Estos atributos hay que crearlos a mano en Brevo** (Contactos → Configuración → Atributos): `PEDIDOS_TOTAL` tipo Número y `ULTIMO_PEDIDO` tipo Fecha. Si no existen, Brevo rechaza el contacto entero.
- **Brevo trata `WHATSAPP` (atributo de tipo teléfono) como identificador ÚNICO.** Si el número ya está en otro contacto, rechaza el alta ENTERA con `400 duplicate_parameter`, no solo ese campo. Pasa en la vida real: alguien que se inscribió a un taller y luego pide un producto con otro correo, o dos personas de una familia que comparten número. `upsertBrevoContact` ya lo maneja: reintenta sin `WHATSAPP` y guarda el contacto igual (el número viaja de todos modos en el correo de aviso interno, con su botón de WhatsApp).
