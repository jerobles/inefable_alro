...
0.2. **Carrito de compras (2026-09-05).** Antes solo se podía pedir un producto por vez: tres velas distintas eran tres formularios, tres pagos y **tres domicilios**. Por módulos, como siempre:
   ...
   - **Carrito-3b** ✅ (2026-09-06) — el usuario confirmó el carrito en producción y se retiró `FormPedido` de las páginas de producto (`tienda/[slug].astro`), junto con el enlace "Prefiero coordinarlo por WhatsApp" que apuntaba a ese formulario. **`/tienda` (el catálogo) SÍ lo conserva** — no se tocó porque el usuario pidió solo las páginas de producto; queda pendiente preguntarle si también lo quita de ahí. Por eso `pago-producto.js` y `producto-sync.js` mantienen la compatibilidad con el payload de un producto suelto: sigue habiendo un formulario que la usa.
