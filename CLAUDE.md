...
0.9. **Pendientes del usuario (no van en la página, son tareas suyas).** Lista consolidada al cierre de la sesión del 2026-09-06 — el sitio quedó **funcionando y vendiendo**, con la primera venta real hecha y verificada en los cinco puntos (correo al cliente, aviso interno, hoja de Google, Brevo y Mercado Pago).
   ...
   - **Activar el webhook de Mercado Pago** — implementado y **desplegado el 2026-09-11**. Estado de los 3 pasos del usuario:
     1. ⏳ **LO ÚNICO QUE FALTA:** la **clave secreta** del panel de Mercado Pago en Netlify como `MP_WEBHOOK_SECRET`. El usuario la está esperando de quien administra esa cuenta. Mientras no esté, el webhook rechaza todo aviso con `401` y el log dice exactamente `Firma rechazada: MP_WEBHOOK_SECRET no está configurado` — es a propósito: sin poder verificar la firma no hay forma de distinguir un aviso real de uno inventado, así que falla cerrado.
     2. ✅ Variables creadas (incluido el atributo `ULTIMO_PAGO` en Brevo).
     3. ✅ Apps Script de la hoja actualizado **y reimplementado** (2026-09-11).
     - **Mientras tanto no se pierde nada:** el pago queda igual en el panel de Mercado Pago y la hoja sigue diciendo "En línea (confirmar en Mercado Pago)" — o sea, el comportamiento manual de siempre. Sigue vigente la regla de **confirmar cada pago en el panel por número de pedido antes de despachar**.
     - Ojo: desde este despliegue el sitio manda `notification_url` en cada preferencia, así que **Mercado Pago ya está notificando** y esos avisos se rechazan con 401 hasta que llegue la clave. Ver 401s en el log del webhook antes de configurarla es lo esperado, no un fallo.
