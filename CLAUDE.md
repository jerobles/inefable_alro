...
0.3. **Pago en línea con Mercado Pago (2026-08-22, código completo con datos de prueba — falta el Access Token real para probar de punta a punta).** Ver [[pasarela-pago-mercado-pago]] en memoria: el proveedor real es Mercado Pago (no Wompi), misma cuenta que ya usa el cliente para el curso.
   ...
   - **✅ Webhook de confirmación de pago IMPLEMENTADO (2026-09-11)** — `netlify/functions/pago-webhook.js` + `lib/firma-mp.js` + `actualizarPagoEnHoja()` en `lib/hoja.js`. Ver `docs/webhook-mercado-pago.md` para las decisiones de diseño y la tabla de casos probados (9 escenarios en dry-run del bundle CJS, todos pasando). **Falta que el usuario haga los 3 pasos de configuración y la prueba con un pago real.**
...
0.9. **Pendientes del usuario (no van en la página, son tareas suyas).** Lista consolidada al cierre de la sesión del 2026-09-06 — el sitio quedó **funcionando y vendiendo**, con la primera venta real hecha y verificada en los cinco puntos (correo al cliente, aviso interno, hoja de Google, Brevo y Mercado Pago).
   ...
   - **Activar el webhook de Mercado Pago** (ya implementado el 2026-09-11, faltan 3 pasos suyos — la guía está en `docs/webhook-mercado-pago.md`):
     1. Registrar la URL en el panel de Mercado Pago y poner la clave secreta en Netlify como `MP_WEBHOOK_SECRET`.
     2. Crear el atributo `ULTIMO_PAGO` (Fecha) en Brevo.
     3. Volver a implementar el Apps Script de la hoja con la versión nueva (crear **implementación nueva**, no solo guardar).
     Hasta que eso esté y se pruebe con un pago real, sigue vigente la regla de **confirmar cada pago en el panel por número de pedido antes de despachar**.
